...
int main(void)
{
	...
	do{
        ...
        //glPolygonMode(GL_FRONT_AND_BACK,GL_LINE); // Wireframe bug :D
		...
		//use the space key to remove points
		if(glfwGetKey(g_pWindow,  GLFW_KEY_SPACE) == GLFW_PRESS){
			foo.removeUm();	
    		indices = foo.returnInd();
    		vertices = foo.returnVert();
		}

		//use the backspace key to add points
		...
		cout << indices.size() << "==" << vertices.size() << "\n";
