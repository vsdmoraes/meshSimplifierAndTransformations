...
double sheY[3] = {0,0,1};
double sheZ[3] = {0,0,1};
glm::vec3 eye = glm::vec3(sheX[0],sheX[1],sheX[2]);
glm::vec3 center = glm::vec3(sheY[0],sheY[1],sheY[2]);
glm::vec3 up = glm::vec3(sheZ[0],sheZ[1],sheZ[2]);
...
void TW_CALL resetAll(void *){ 
	...
	eye = glm::vec3(sheX[0],sheX[1],sheX[2]);
	center = glm::vec3(sheY[0],sheY[1],sheY[2]);
	up = glm::vec3(sheZ[0],sheZ[1],sheZ[2]);
...
int main(void) {
	...
	// LOOK AT
	TwAddVarRW(g_pToolBar, "Can Look At", TW_TYPE_BOOLCPP, &canLookAt, "group=LookAt label='Can Look At'");
	TwAddVarRW(g_pToolBar, "Eye", TW_TYPE_DIR3D, &sheX, "group=LookAt label='Eye'");
	TwAddVarRW(g_pToolBar, "Center", TW_TYPE_DIR3D, &sheY, "group=LookAt label='Center'");
	TwAddVarRW(g_pToolBar, "Up", TW_TYPE_DIR3D, &sheZ, "group=LookAt label='Up'");
	...
	do{
		...
		if (currentTime - lastTime >= 1.0){ // If last prinf() was more than 1sec ago
			...
			if(canTranslate!=false && times > 0){
				ViewMatrix = foo.translating(ViewMatrix, traX, traY, traZ);
				traX*=1.1;
				traY*=1.1;
				traZ*=1.1;
				times-=1.0;
			...
			if(canRotate!=false && aroundPoint!=false && times > 0){
				ViewMatrix = foo.rotatingPoint(ViewMatrix, rotX, rotY, rotZ, pX, pY, pZ, previRotX, previRotY, previRotZ);
				previRotX = pX;
				previRotY = pY;
				previRotZ = pZ;
				rotX*=1.4;
				...
				if(canRotate!=false && times > 0){
					ViewMatrix = foo.rotating(ViewMatrix, rotX, rotY, rotZ);
					rotX*=1.4;
					rotY*=1.3;
					rotZ*=1.2;
					times-=1.0;
		...
		if(canLookAt!=false){
			...
			center = glm::vec3(sheY[0],sheY[1],sheY[2]);
			up = glm::vec3(sheZ[0],sheZ[1],sheZ[2]);
			ViewMatrix = foo.lookingAt(eye, center, up);
		}
